document.addEventListener("DOMContentLoaded", function () {
	...
	const departureCitySuggestions = document.getElementById(
	...
	const arrivalCitySuggestions = document.getElementById(
		"arrival-city-suggestions"
	);
	...
	function openModalWithData(tripData) {
		...
		const driverNameElement = modalContent.querySelector(
			".custom-driver-info h2 a"
		);
		...
		const driverDescriptionElement = modalContent.querySelector(
			".custom-driver-info p"
		);
		if (driverDescriptionElement) {
			driverDescriptionElement.textContent = tripData.driver_description;
		}

		const driverPhotoElement = modalContent.querySelector(
			".custom-driver-photo"
		);
		if (driverPhotoElement) {
			const defaultPhotoUrl = "/static/img/defaul.png"; // Обратите внимание на путь, он должен быть правильным
			const photoUrl = tripData.driver_photo_url
				? tripData.driver_photo_url
				: defaultPhotoUrl;
		...
		const departureAddressElement = modalContent.querySelector(
			".custom-from p:nth-child(2)"
		);
		if (departureAddressElement) {
			departureAddressElement.textContent = tripData.departure_address;
		}

		const departureDateElement = modalContent.querySelector(
			".custom-from p:nth-child(3)"
		);
		if (departureDateElement) {
			departureDateElement.textContent = `${tripData.departure_date}, ${tripData.departure_time}`;
		}

		const passengersElement = modalContent.querySelector(
			".custom-from p:nth-child(4)"
		);
		if (passengersElement) {
			passengersElement.textContent = `${tripData.passengers} пассажира`;
		}

		const destinationAddressElement = modalContent.querySelector(
			".custom-to p:nth-child(2)"
		);
		if (destinationAddressElement) {
			destinationAddressElement.textContent = tripData.destination_address;
		}

		const arrivalDateElement = modalContent.querySelector(
			".custom-to p:nth-child(3)"
		);
